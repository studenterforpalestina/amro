...
<nav>
    <div class="flex items-center justify-between px-10 py-6">
        ...
        <ul class="flex list-none items-center gap-10">
            <li>
                <button class="bg-transparent border-0 cursor-pointer px-3 py-2 rounded-md hover:text-red-600 hover:bg-red-100 transition-colors duration-200" on:click={() => (dark = !dark)}>
                    {#if dark}
                        <Moon />
                    {:else}
                        <Sun />
            ...
            {#each data as item}
                <li>
                    <a href={item.url} class="flex text-black text-lg px-3 py-2 rounded-md hover:text-red-600 hover:bg-red-100 transition-colors duration-200">
                        {item.string} 
                    </a>
                </li>
            {/each}

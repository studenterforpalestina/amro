...
<nav class="flex items-center justify-between px-10 py-6 gap-2 sticky top-0 left-0 w-full bg-(--color-bg-light)">
    <div class="flex w-full justify-between items-center">
        <a href="/">
            <img class="w-16" src="/images/logo.png" alt="logo" />
        </a>
        <button class="cursor-pointer text-red-600 px-3 py-2 rounded-md
    ...
    <ul class="list-none items-center gap-1 hidden sm:flex">
        ...
        {#each data as item}
            <li>
                <a
                    href={item.url}
                    class="flex text-black px-3 py-2 rounded-md
    ...
    <div class={c('transition-transform z-50 fixed top-0 left-full h-dvh bg-(--color-bg-light) px-3 py-5 sm:hidden', menuOpen && 'transform-[translateX(-100%)]')}>
        <ul class="list-none flex flex-col items-start p-4 gap-2 min-w-60">
            <li class="flex w-full justify-between items-center gap-1 mb-4">
                <button class="cursor-pointer text-red-600 pl-2 pr-5 py-2 rounded-md
                hover:bg-red-100 active:bg-red-200
